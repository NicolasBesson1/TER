...
class Exists:
    ...
    def cooper(self,x=None):
        ...
        if d!=None and dp!=None:
            S=[]
            for i in range(1,d+1):
                for t in T:
                    #Compute P(x)[t+i \ ax] for each i from 1 to d, and for each ax in such that P(x) contains ax < t
                    t=t.add(Factor(var=None,const=i))
                    r=self.constraint.replaceax(t)
                    if r!=None:
                        S.append( Junction( AND , [ r, DivConstraint( Sum ( [ Factor ( dp ) ] ), t ) ] ) )
            f=Junction(OR,[f] + S)
